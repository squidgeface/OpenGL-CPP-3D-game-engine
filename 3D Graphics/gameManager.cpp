...
CGameManager::~CGameManager()
{
	...
	while (m_vArenaWallVec.size() > 0)
	{
		CPrefab* aWall = m_vArenaWallVec[m_vArenaWallVec.size() - 1];
		m_vArenaWallVec.pop_back();
		delete aWall;
	}
...
//Load shapes
	...
	m_pArena->Initialise(m_pProjCamera, m_pTime, m_pInput, MeshType::MODEL, "Resources/Models/City/Camellia City.obj", 0, vec3(0.01f, 0.01f,0.01f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, -100.0f, 0.0f));
	//m_pArena->SetLightPosition(vec3(m_pArena->GetObjPosition().x, m_pArena->GetObjPosition().y + 1000.0f, m_pArena->GetObjPosition().z));
	//game shapes
...
//load textures 
	...
	//set up arena walls
	m_pWall = new CPrefab();
	m_pWall->Initialise(m_pProjCamera, m_pTime, m_pInput, MeshType::CUBE, "", 0, vec3(200.0f, 10.0f, 10.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 0.0f, -100.0f)); //left
	m_vArenaWallVec.push_back(m_pWall);
	m_pWall = new CPrefab();
	m_pWall->Initialise(m_pProjCamera, m_pTime, m_pInput, MeshType::CUBE, "", 0, vec3(200.0f, 10.0f, 10.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 0.0f, 100.0f)); //right
	m_vArenaWallVec.push_back(m_pWall);
	m_pWall = new CPrefab();
	m_pWall->Initialise(m_pProjCamera, m_pTime, m_pInput, MeshType::CUBE, "", 0, vec3(10.0f, 10.0f, 190.0f), vec3(0.0f, 0.0f, 0.0f), vec3(70.0f, 0.0f, 0.0f)); //bottom
	m_vArenaWallVec.push_back(m_pWall);
	m_pWall = new CPrefab();
	m_pWall->Initialise(m_pProjCamera, m_pTime, m_pInput, MeshType::CUBE, "", 0, vec3(10.0f, 10.0f, 190.0f), vec3(0.0f, 0.0f, 0.0f), vec3(-95.0f, 0.0f, 0.0f)); //top	
	m_vArenaWallVec.push_back(m_pWall);
	//load in textures to walls
	for (size_t i = 0; i < m_vArenaWallVec.size(); i++)
	{
		m_vArenaWallVec[i]->InitialiseTextures("Resources/Textures/wallBlock.png", 1);
	}
...
void CGameManager::Render()
{
	...
	//render vectors for enemies and bullets
		...
		for (size_t i = 0; i < m_vArenaWallVec.size(); i++)
		{
			m_vArenaWallVec[i]->RenderShapes(m_giBlinnProgram);
		}
...
//switch for game state
	...
	case GameState::GAME:
		...
		if (!m_bPaused)
		{
			...
			m_pPointer->SetObjPosition((float)m_pInput->GetMouseY() * 0.5f, -10.0f, -(float)m_pInput->GetMouseX() * 0.5f);

			//move player on input
			PlayerRun();
			//Set camera looking at the arena
			...
			for (size_t i = 0; i < m_vArenaWallVec.size(); i++)
			{
				m_vArenaWallVec[i]->UpdateShapes();
			}
			//update disco ball
			for (size_t i = 0; i < m_vDiscoVec.size(); i++)
			{
...
void CGameManager::PlayerRun()
{
	...
	m_pPlayerPrefab->PlayerMovement(vec3(m_fvertical * m_fplayerSpeed, 0.0f, m_fhorizontal * m_fplayerSpeed), m_pTime);
}
...
void CGameManager::EnemySpawner()
{
	...
	if ((int)m_vEnemySpriteVec.size() < m_ispawnSize)
	{
		...
		if (m_fspawnCount > m_fspawnRate)
		{
			...
			CEnemy* enemySprite = new CEnemy((int)m_pPlayerPrefab->GetObjPosition().x, (int)m_pPlayerPrefab->GetObjPosition().y);
			//choose randomn location on screen
			srand((unsigned int)(rand() ^ time(NULL)));
			float xrand = rand() % 130 - 70.0f; //top/bottom
			float zrand = rand() % 165 - 85.0f; //left/right
			enemySprite->Initialise(m_pProjCamera, m_pTime, m_pInput, MeshType::BLANK, "", 11.0F, vec3(0.1f, 0.1f, 0.1f), vec3(0.0f, 0.0f, 0.0f), vec3(xrand, 45.0f, zrand));
...
void CGameManager::BulletEnemyCollision()
{
	...
	for (size_t j = 0; j < m_vArenaWallVec.size(); j++)
	{
		for (size_t i = 0; i < m_vBullets.size(); i++)
		{
			if (m_vBullets[i]->CheckObjectCollision(m_vArenaWallVec[j]))
			{
				CBullet* ABullet = m_vBullets[i];
				m_vBullets.erase(m_vBullets.begin() + i);
				delete ABullet;
...
void CGameManager::ProcessInput(InputState* KeyState, InputState* MouseState)
{
	...
	case GameState::GAME:
		...
		if (!m_bPaused)
		{
			...
			//Left wall collision
			if (m_pPlayerPrefab->GetObjPosition().z <= 90)
			{
				m_bgoingL = true;
			}
			else
			...
			//Top wall collision
			if (m_pPlayerPrefab->GetObjPosition().x >= -85)
			{
				m_bgoingU = true;
			}
			else
			...
			if (m_pPlayerPrefab->GetObjPosition().x <= 60)
			{
				m_bgoingD = true;
			}
			else
